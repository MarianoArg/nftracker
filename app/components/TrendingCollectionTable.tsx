...
import { PercentageChange } from "~/components/HeroStats";
...
type Volumes = "1DayVolume" | "7DayVolume" | "30DayVolume" | "allTimeVolume";

const TrendingCollectionTable: FC<Props> = ({ fallback }) => {
  const [searchParams] = useSearchParams();
  const isSmallDevice = useMediaQuery("only screen and (max-width : 600px)");
  const sort: Volumes | null = searchParams.get("sort") as Volumes;
  const { collections, ref } = usePaginatedCollections(fallback?.collections);
            ...
            return (
              ...
              >
                ...
                <td className="reservoir-body flex items-center gap-4 whitespace-nowrap px-6 py-4 dark:text-white">
                  ...
                  <Link to={tokenHref} className="flex items-center gap-2">
                    <img
                      alt={`${name} logo`}
                      src={image}
